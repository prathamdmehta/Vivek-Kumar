...
const generateAnimationDuration = (index) => {
  // Generate random but deterministic duration based on skill index
  // This ensures consistent animations per skill across re-renders
          ...
          {skills.map((skill, index) => (
            <motion.div
              ...
              animate={{
                ...
                x: [0, 10, 0],  // Subtle side drift
                rotate: [0, 5, -5, 0]
              }}
              transition={{
                duration: generateAnimationDuration(index), // Deterministic speed per skill
                repeat: Infinity,
                ease: "easeInOut"
              }}
              whileHover={{ 
