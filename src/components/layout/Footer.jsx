  ...
  return (
    <footer className="w-full p-10 px-6 bg-transparent font-sans">
      <div className="max-w-5xl mx-auto">
        <motion.div initial={{ y: 40, opacity: 0 }} whileInView={{ y: 0, opacity: 1 }} viewport={{ once: true }} className="relative grid grid-cols-1 md:grid-cols-3 items-center gap-6 p-3 rounded-[2.5rem] border border-white/10 bg-zinc-900/50 backdrop-blur-2xl shadow-2xl">
          ...
          <div className="flex justify-self-center items-center bg-zinc-800/50 border border-white/5 p-1.5 rounded-full">
            {[
              ...
              <motion.a key={i} href={social.href} whileHover={{ y: -3, scale: 1.1, backgroundColor: "rgba(255,255,255,0.05)" }} className="p-3 rounded-full text-zinc-400 hover:text-white transition-all">
                {social.icon}
              </motion.a>
            ))}
            <div className="w-px h-4 bg-zinc-700 mx-2" />
            <motion.button onClick={handleCopy} whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} className="flex items-center gap-2 px-4 py-2 rounded-full text-sm font-medium text-zinc-300 hover:text-white">
              <AnimatePresence mode="wait">
                {copied
                  ? <motion.span key="check" initial={{ scale: 0 }} animate={{ scale: 1 }} exit={{ scale: 0 }}><Check size={16} className="text-emerald-400" /></motion.span>
          ...
          <div className="justify-self-center md:justify-self-end pr-2">
            <motion.a href="#contact" whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} className="group flex items-center gap-3 bg-indigo-600 hover:bg-indigo-500 text-white pl-6 pr-3 py-3 rounded-full text-sm font-bold transition-all shadow-lg shadow-indigo-500/20">
              START A PROJECT
